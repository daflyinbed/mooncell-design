<template>
  <Primitive
    as="div"
    :class="`m-BaseCard m-Card m-variant-${variant} m-size-${size}`"
    :as-child="asChild"
  >
    <slot></slot>
  </Primitive>
...
.m-Card:where(.m-variant-surface) {
...
.m-Card:where(.m-variant-classic) {
  --card-border-width: 1px;
  --card-background-color: var(--color-panel);
...
.m-Card:where(:any-link, button, label) {
